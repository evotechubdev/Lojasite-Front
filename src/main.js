...
function renderPortal() {
  ...
  const showcaseCards=[1,2,3].map(group=>`<div class="portal-showcase-group row g-3" aria-label="Grupo ${group} de 3">${showcaseProducts.filter(product=>product.group===group).map(product=>`<div class="col-12 col-sm-6 col-lg-3"><article class="portal-product-card"><div class="portal-product-art"><span>${escapeHtml(product.category)}</span><img src="${import.meta.env.BASE_URL}imagens_pub/vitrine/${encodeURIComponent(product.image)}" alt="${escapeHtml(product.name)}" loading="lazy"></div><div><h3>${escapeHtml(product.name)}</h3><strong>${money(product.price)}</strong><button type="button">Ver ${product.service?'serviço':'produto'}</button></div></article></div>`).join('')}</div>`).join('');

...
function renderApp() {
  ...
  const contactItems = `<div class="store-hours"><b>Horário de funcionamento</b><span>${escapeHtml(info.hours || 'Consulte a loja')}</span><div class="contact-icons">${info.whatsapp ? `<a href="https://wa.me/${escapeHtml(info.whatsapp)}" target="_blank" rel="noopener" aria-label="Abrir WhatsApp" title="WhatsApp"><span aria-hidden="true">◉</span></a>` : ''}${info.email ? `<a href="mailto:${escapeHtml(info.email)}" aria-label="Enviar e-mail" title="E-mail"><span aria-hidden="true">✉</span></a>` : ''}</div></div>`;
  $('#app').innerHTML = `<header class="header store-header"><a class="logo store-logo store-logo-only" href="${import.meta.env.BASE_URL}${slug}" aria-label="${escapeHtml(store.name)}"><img id="store-logo-image" src="${import.meta.env.BASE_URL}imagens/${store.id}/logo.png" alt="Logo ${escapeHtml(store.name)}"></a><div class="header-search-wrap"><label class="header-search">⌕<input id="header-search" value="${escapeHtml(state.query)}" autocomplete="off" placeholder="Pesquisar produtos" aria-label="Pesquisar produtos" aria-controls="search-suggestions"></label><div class="search-suggestions" id="search-suggestions" hidden></div></div><nav class="store-nav" aria-label="Seções da loja"><a href="#novidades" data-store-section="novidades">NOVIDADES</a><a href="#produtos" data-store-section="produtos">PRODUTOS</a><a href="#nossa-loja" data-store-section="nossa-loja">NOSSA LOJA</a></nav><div class="header-actions">${state.user ? `<div class="user-menu-wrap"><button class="logged-user" id="user-menu-button" aria-label="Abrir menu de ${escapeHtml(state.user.name)}" aria-expanded="false"><span class="user-avatar">${userAvatar}</span><span class="logged-user-name">${escapeHtml(state.user.name)}</span><b>⋮</b></button><nav class="user-menu" id="user-menu" hidden><strong>${escapeHtml(state.user.name)}</strong><small>${escapeHtml(state.user.cargo || '')}</small>${staff ? `<button data-corporate="stock">Estoque</button>${manager ? `<button data-corporate="payments">Sistema de Pagamento</button><button data-corporate="access">Gestão de Logins</button>` : ''}` : `<button id="account">Dados Cadastrais</button><button id="payment-data">Dados de Pagamento</button><button id="orders-menu">Minhas Compras</button>`}<button id="logout-menu">Sair</button></nav></div>` : `<button class="signup-button" id="signup">Cadastre-se</button><button class="login-button" id="login">Entrar</button>`}<button class="cart-button" id="cart" aria-label="Abrir carrinho">${icon('bag',22)}<span>Carrinho</span><b id="cart-count" hidden>0</b></button></div></header>
  <main><section class="hero store-page-section" id="novidades"><div class="hero-copy"><span class="kicker">NOVIDADES</span><h1>${escapeHtml(store.tagline)}</h1><p>Escolhas especiais, compra segura e entrega para todo o Brasil.</p><a href="#produtos" data-store-section="produtos">Ver produtos <span>↗</span></a></div>${heroVisual}</section>
  <section class="catalog store-page-section" id="produtos"><div class="section-head"><div><span class="kicker">PRODUTOS</span><h2>Produtos em destaque</h2></div><button class="more-products" id="more-products" type="button" aria-expanded="false">Mais Produtos</button></div><div class="catalog-browser"><aside class="category-panel"><div class="category-navigation" aria-label="Categorias">${categories.map(category=>`<button data-category="${escapeHtml(category)}" class="${state.category===category?'active':''}">${category==='Todos'?'Todas as Categorias':escapeHtml(category)}</button>`).join('')}</div></aside><div class="product-grid" id="products"></div></div></section>
  <section class="store-about store-page-section" id="nossa-loja"><div class="store-about-copy"><span class="kicker">NOSSA LOJA</span><h2>${escapeHtml(store.name)}</h2><button class="store-description" id="store-description" type="button">${escapeHtml(info.description || store.tagline)}<span>...mais</span></button><div class="store-contacts">${contactItems}</div></div><div class="store-map-column"><div class="store-map">${info.mapUrl || generatedMapUrl ? `<iframe src="${escapeHtml(info.mapUrl || generatedMapUrl)}" title="Mapa de ${escapeHtml(store.name)}" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>` : '<div><strong>Mapa em preparação</strong><span>O endereço desta loja será publicado em breve.</span></div>'}</div>${address ? `<div class="store-route-actions"><button id="full-address" type="button">Endereço Completo</button><a href="${directionsLink}" target="_blank" rel="noopener">Como Chegar</a></div>` : ''}</div><footer class="portal-simple-footer organization-footer"><span class="footer-system-logo"><img src="${import.meta.env.BASE_URL}imagens/logo.png" alt="Logo LojaSite"></span><button class="footer-developer" id="developer-contact" type="button" aria-label="Abrir contatos de suporte da EVOTECHUB"><img src="${import.meta.env.BASE_URL}imagens/logo_dev.png" alt="Logo EVOTECHUB"><span>© EVOTECHUB 2026 - Todos os direitos reservados.</span></button></footer></section></main>`;
  ...
  $('#login')?.addEventListener('click', () => openLogin()); $('#signup')?.addEventListener('click', openRegistration); $('#account')?.addEventListener('click', openAccount); $('#payment-data')?.addEventListener('click', openPaymentData); $('#orders-menu')?.addEventListener('click', showOrders); $('#cart').onclick = openCart; $('#header-search').oninput = e => { state.query = e.target.value; state.productsExpanded = false; renderProducts(); renderSearchSuggestions(e.target.value); };
  ...
  $('#full-address')?.addEventListener('click',()=>modal(`<span class="kicker">NOSSA LOJA</span><h2>Endereço Completo</h2><p class="address-modal-text">${escapeHtml(address)}</p>`));

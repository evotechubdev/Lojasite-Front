...
async function openDeveloperContact() {
  try {
    ...
    overlay.innerHTML=`<section class="mini-modal developer-popup" role="dialog" aria-modal="true" aria-labelledby="developer-popup-title"><button class="mini-modal-close close" type="button" aria-label="Fechar">×</button><div class="developer-popup-logos"><img src="${import.meta.env.BASE_URL}imagens/logo.png" alt="Logo ${escapeHtml(systemConfig.name)}"><span></span><img src="${import.meta.env.BASE_URL}imagens/logo_dev.png" alt="Logo EVOTECHUB"></div><span class="kicker">SUPORTE DAS LOJAS</span><h3 id="developer-popup-title">Fale com a EVOTECHUB</h3><div class="developer-popup-actions">${email}${whatsapp}</div></section>`;
    $('#modal-root').append(overlay);
    const closePopup=()=>overlay.remove();
    overlay.querySelector('.mini-modal-close').onclick=closePopup;
    overlay.onclick=event=>{if(event.target===overlay)closePopup();};
...
function openLojaSitePopup(){
  ...
  overlay.innerHTML=`<section class="mini-modal lojasite-popup" role="dialog" aria-modal="true" aria-labelledby="lojasite-popup-title"><button class="mini-modal-close close" type="button" aria-label="Fechar">×</button><img src="${import.meta.env.BASE_URL}imagens/logo.png" alt="Logo ${escapeHtml(systemConfig.name)}"><h3 id="lojasite-popup-title">${escapeHtml(systemConfig.name.toLocaleUpperCase('pt-BR'))}</h3><p>${escapeHtml(systemConfig.slogan)}</p><button class="lojasite-portal-button" type="button">Portal ${escapeHtml(systemConfig.name)}</button></section>`;
  $('#modal-root').append(overlay);
  const closePopup=()=>overlay.remove();
  overlay.querySelector('.mini-modal-close').onclick=closePopup;
  overlay.onclick=event=>{if(event.target===overlay)closePopup();};

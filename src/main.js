...
function productCard(product) {
  ...
  const alreadyInCart=Math.max(0,Math.floor(Number(state.cart[product.id])||0));
  ...
  const quantityControl=available?`<div class="product-quantity" aria-label="Quantidade de ${escapeHtml(product.name)}"><button data-product-minus="${product.id}" type="button" aria-label="Diminuir quantidade">−</button><span><b data-product-quantity="${product.id}">1</b>/${available}</span><button data-product-plus="${product.id}" type="button" aria-label="Aumentar quantidade">+</button></div>`:`<div class="product-quantity sold-out">0/${stock} disponível</div>`;
  return `<article class="product-card"><div class="product-art" style="--art:${product.color}"><span class="product-tag">${escapeHtml(product.category)}</span>${product.imageUrl ? `<img src="${escapeHtml(product.imageUrl)}" alt="${escapeHtml(product.name)}" loading="lazy">` : icon(product.icon, 72)}</div><div class="product-info"><h3>${escapeHtml(product.name)}</h3>${descriptionElement}<div class="product-buy"><div class="product-actions"><button class="request-purchase" data-request-purchase="${product.id}" type="button">${whatsappIcon}<span>Solicitar Compra</span></button><div class="product-add-row">${quantityControl}<button class="add" data-add="${product.id}" ${available?'':'disabled'} aria-label="Adicionar ${escapeHtml(product.name)}">${icon('bag',20)}<span>${available?'Adicionar 1':'Sem estoque'}</span></button></div></div>${priceBlock}</div></div></article>`;
}
function renderProducts() {
  ...
  const mobileLayout = matchMedia('(max-width: 600px)').matches;
  const initialProductLimit = matchMedia('(max-width: 900px)').matches ? 2 : 3;
  const visibleProducts = mobileLayout ? products.slice(0, 4) : (state.productsExpanded ? products : products.slice(0, initialProductLimit));
  ...
  if (moreButton) {
    moreButton.hidden = mobileLayout || products.length <= initialProductLimit;
  ...
  document.querySelectorAll('[data-product-minus]').forEach(button=>button.onclick=()=>{const value=document.querySelector(`[data-product-quantity="${button.dataset.productMinus}"]`);const next=Math.max(1,Number(value.textContent)-1);value.textContent=next;document.querySelector(`[data-add="${button.dataset.productMinus}"] span`).textContent=`Adicionar ${next}`;});
  document.querySelectorAll('[data-product-plus]').forEach(button=>button.onclick=()=>{const value=document.querySelector(`[data-product-quantity="${button.dataset.productPlus}"]`);const limit=Number(value.parentElement.textContent.split('/')[1]);const next=Math.min(limit,Number(value.textContent)+1);value.textContent=next;document.querySelector(`[data-add="${button.dataset.productPlus}"] span`).textContent=`Adicionar ${next}`;});
  document.querySelectorAll('[data-add]').forEach(btn => btn.onclick = () => {
    ...
    const product=state.store.products.find(item=>item.id===btn.dataset.add);
    const stock=Math.max(0,Math.floor(Number(product?.stock)||0));
    const current=Math.max(0,Math.floor(Number(state.cart[btn.dataset.add])||0));
    const requested=Number(document.querySelector(`[data-product-quantity="${btn.dataset.add}"]`)?.textContent||0);
    if(!product||requested<1||current+requested>stock)return toast('A quantidade solicitada não está disponível no estoque.','error');
    state.cart[btn.dataset.add] = current + requested;
    saveCart(); toast(`${requested} ${requested===1?'unidade adicionada':'unidades adicionadas'} ao carrinho.`); btn.classList.add('added');
...
function renderApp() {
  ...
  const syncStoreHeaderHeight=()=>document.documentElement.style.setProperty('--store-header-height',`${Math.ceil(storeHeader.getBoundingClientRect().height)}px`);
  syncStoreHeaderHeight();
  new ResizeObserver(syncStoreHeaderHeight).observe(storeHeader);
  ...
  const setUserMenu = open => {
    if (!userMenu) return;
    if (open) { userMenu.style.top='12px';userMenu.style.maxHeight='calc(100svh - 24px)'; }
...
async function openCamera({ facingMode='user', onCapture }) {
  if (!navigator.mediaDevices?.getUserMedia) return toast('A câmera não está disponível neste navegador.','error');
  const layer=document.createElement('div');
  layer.className='camera-capture-overlay';
  layer.innerHTML=`<section class="camera-capture-modal" role="dialog" aria-modal="true" aria-label="Câmera"><button class="close camera-close" type="button" aria-label="Fechar">×</button><h2>Tirar foto</h2><video autoplay playsinline muted></video><p class="camera-help">Posicione a imagem e toque em capturar.</p><button class="primary full camera-shutter" type="button">Capturar foto</button></section>`;
  document.body.append(layer);
  const video=layer.querySelector('video');
  let stream;
  const finish=()=>{stream?.getTracks().forEach(track=>track.stop());layer.remove();};
  layer.querySelector('.camera-close').onclick=finish;
  ...
  try {
    ...
    return toast(denied?'Permita o acesso à câmera para tirar a foto.':'Não foi possível abrir a câmera.','error');
  }
  layer.querySelector('.camera-shutter').onclick=async()=>{
...
function renderPortal() {
  ...
  const showcaseWhatsappIcon='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20.5 11.6a8.5 8.5 0 0 1-12.6 7.5L3 20.5l1.4-4.7A8.5 8.5 0 1 1 20.5 11.6Z"/><path d="M8.2 7.8c.3-.4.6-.4.9-.1l1.1 2c.2.4 0 .7-.4 1.1-.2.2.8 2 2.7 2.8.5.2.8-.6 1.2-1 .2-.2.5-.2.8 0l1.8 1.1c.4.2.4.6.3.9-.5 1.2-1.4 1.8-2.7 1.7-3.5-.3-7-3.5-7.2-6.7-.1-.8.5-1.5 1.5-1.8Z"/></svg>';
  const showcaseProductCard=product=>`<article class="portal-product-card"><div class="portal-product-art"><span>${escapeHtml(product.category)}</span><img src="${import.meta.env.BASE_URL}imagens_pub/vitrine/${encodeURIComponent(product.image)}" alt="${escapeHtml(product.name)}" loading="lazy"></div><div class="portal-product-info"><h3>${escapeHtml(product.name)}</h3><div class="portal-product-buy"><div class="product-actions"><button class="request-purchase" data-demo-action="whatsapp" data-demo-product="${escapeHtml(product.name)}" type="button">${showcaseWhatsappIcon}<span>Solicitar Compra</span></button><button class="add" data-demo-action="cart" data-demo-product="${escapeHtml(product.name)}" type="button">${icon('bag',20)}<span>Adicionar</span></button></div><div class="product-price"><strong>${money(product.price)}</strong></div></div></div></article>`;
  const showcaseCards=[1,2,3].map(group=>`<div class="portal-showcase-group row g-3" aria-label="Grupo ${group} de 3">${showcaseProducts.filter(product=>product.group===group).map(product=>`<div class="col-lg-3">${showcaseProductCard(product)}</div>`).join('')}</div>`).join('');
  const mobileShowcaseRows=[showcaseProducts.slice(0,6),showcaseProducts.slice(6,12)].map((products,row)=>`<div class="mobile-showcase-row"><button class="mobile-showcase-previous" type="button" aria-label="Produto anterior da faixa ${row+1}">←</button><div class="mobile-showcase-track" data-mobile-showcase="${row}">${products.map(product=>`<div class="mobile-showcase-slide">${showcaseProductCard(product)}</div>`).join('')}</div><button class="mobile-showcase-next" type="button" aria-label="Próximo produto da faixa ${row+1}">→</button></div>`).join('');
  const plans=systemConfig.plans.map(plan=>`<div class="col-12 col-md-4"><article class="portal-plan${plan.recommended?' featured':''}"><h3>${escapeHtml(plan.name)}</h3><div class="plan-stars" aria-label="${plan.stars} de 5 estrelas">${'★'.repeat(plan.stars)}<i>${'★'.repeat(5-plan.stars)}</i></div><div class="plan-price">${plan.startingAt?'<small>A partir de</small>':''}<small>R$</small><strong>${plan.monthlyPrice.toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2})}</strong><small>/mês</small></div><ul>${plan.benefits.map(item=>`<li>${escapeHtml(item)}</li>`).join('')}</ul><a href="#planos-contato" data-plan-contact="${escapeHtml(plan.name)}">Quero esse plano</a></article></div>`).join('');
  ...
  <main>
  <section class="portal-hero portal-page-section" id="sua-loja"><div><span class="kicker">VENDAS QUE GERAM RESULTADOS</span><h1>Tenha sua loja online e venda pela internet agora mesmo</h1><ul class="portal-hero-checklist"><li>Design moderno</li><li>Visibilidade no Google (SEO)</li><li>Integração com WhatsApp</li><li>Carregamento rápido</li><li>Hospedagem segura</li><li>Suporte humanizado</li></ul><a class="portal-primary" href="#vitrine">Conhecer a ${escapeHtml(systemConfig.name)} <span>↓</span></a></div><div class="portal-window"><div class="window-bar"><i></i><i></i><i></i><small>lojasite.com.br/sua-loja</small></div><div class="window-content"><span class="your-logo-placeholder">Sua Logo</span><div><small>SEU NEGÓCIO ONLINE</small><strong>Estoque, vitrine e vendas em <b>um só lugar</b></strong></div></div></div><div class="hero-store-access-block"><div class="store-access-guidance"><span>Digite o complemento do link da loja informado pelo desenvolvedor.</span><button id="access-link-help" type="button">OBS</button></div><form class="hero-store-access" id="portal-access-form"><span>lojasite.com.br/</span><input name="suffix" placeholder="sua_organizacao" pattern="[a-zA-Z0-9_-]+" required><button type="submit">Acessar minha loja</button></form></div></section>
  <section class="portal-section portal-page-section showcase-section" id="vitrine"><div class="portal-title"><span class="kicker">VITRINE</span><h2>Seus produtos, serviços e categorias, personalizados por você</h2><p>Sua loja online com a sua cara. Experimente!</p></div><div class="showcase-commerce-badge">Forneça Produtos ou Serviços Online!</div><div class="portal-showcase-wrap desktop-showcase"><button id="showcase-previous" type="button" aria-label="Grupo anterior">←</button><div class="portal-showcase" id="portal-showcase">${showcaseCards}</div><button id="showcase-next" type="button" aria-label="Próximo grupo">→</button></div><div class="mobile-showcase">${mobileShowcaseRows}</div></section>
  ...
  document.querySelectorAll('[data-demo-action]').forEach(button=>button.addEventListener('click',()=>{
    const whatsapp=button.dataset.demoAction==='whatsapp';
    const product=button.dataset.demoProduct;
    modal(`<span class="kicker">DEMONSTRAÇÃO</span><h2>${whatsapp?'Compra pelo WhatsApp':'Compra pelo site'}</h2><p class="muted">Na loja real, o botão de <strong>${whatsapp?'Solicitar Compra':'Adicionar'}</strong> para o produto <strong>${escapeHtml(product)}</strong> ${whatsapp?'abre uma confirmação e encaminha o pedido para o WhatsApp da organização.':'adiciona o item ao carrinho para o cliente continuar a compra e realizar o pagamento online.'}</p><p class="demo-feature-note">Esta vitrine é apenas demonstrativa e nenhuma compra será iniciada.</p><button class="primary full" type="button" data-close-demo>Entendi</button>`,'result-modal demo-product-modal');
    document.querySelector('[data-close-demo]')?.addEventListener('click',closeModal);
  }));
  ...
  showcase.addEventListener('touchstart',()=>clearInterval(showcaseTimer),{passive:true});
  showcase.addEventListener('touchend',restartShowcase,{passive:true});
  document.querySelectorAll('.mobile-showcase-row').forEach((row,index)=>{
    const track=row.querySelector('.mobile-showcase-track');
    const move=direction=>{
      const slide=track.querySelector('.mobile-showcase-slide');
      if(!slide)return;
      const step=slide.offsetWidth;
      const atEnd=track.scrollLeft+track.clientWidth>=track.scrollWidth-4;
      const atStart=track.scrollLeft<=4;
      track.scrollTo({left:direction>0?(atEnd?0:track.scrollLeft+step):(atStart?track.scrollWidth:track.scrollLeft-step),behavior:'smooth'});
    };
    let timer;
    const start=()=>{clearInterval(timer);timer=setInterval(()=>move(1),3400+(index*500));};
    const pause=()=>clearInterval(timer);
    row.querySelector('.mobile-showcase-previous').onclick=()=>{pause();move(-1);start();};
    row.querySelector('.mobile-showcase-next').onclick=()=>{pause();move(1);start();};
...
function profilePhotoContent(){const initial=escapeHtml(state.user.name?.trim()?.[0]?.toUpperCase()||'U');const hasPhoto=Boolean(state.user.photoUrl);const avatar=hasPhoto?`<img src="${escapeHtml(state.user.photoUrl)}" alt="Foto atual">`:`<span>${initial}</span>`;const photoAction=hasPhoto?`<button class="photo-remove" id="remove-profile-photo" type="button">Remover ou alterar foto</button><small>Para adicionar outra imagem, remova primeiro a foto atual.</small>`:`<div class="photo-actions"><label class="photo-picker" data-label="Escolher foto">Escolher foto<input class="profile-photo-input" type="file" accept="image/png,image/jpeg,image/webp"></label><button class="photo-picker camera-picker profile-camera-button" type="button">Tirar foto</button></div><small>“Escolher foto” abre a galeria; “Tirar foto” abre a câmera frontal.</small>`;return `<div class="profile-photo-editor"><div class="profile-photo-preview" id="profile-photo-preview">${avatar}</div><div><strong>${escapeHtml(state.user.name)}</strong><small>${escapeHtml(state.user.cargo||'')}</small>${photoAction}</div></div>`;}
function bindProfilePhotoActions(){document.querySelectorAll('.profile-photo-input').forEach(input=>input.addEventListener('change',uploadProfilePhoto));document.querySelectorAll('.profile-camera-button').forEach(button=>button.addEventListener('click',()=>openCamera({facingMode:'user',onCapture:uploadProfileImage})));$('#remove-profile-photo')?.addEventListener('click',removeProfilePhoto);}
function openProfilePhotoEditor(){modal(`<span class="kicker">FOTO DO PERFIL</span><h2>Alterar imagem</h2>${profilePhotoContent()}`,'profile-photo-modal');bindProfilePhotoActions();}
async function openAccount() { const customer=state.user.role==='cliente';let address={};if(customer){try{address=(await api(`/api/stores/${slug}/customer/profile`)).address||{};}catch(error){toast(error.message,'error');}}modal(`<span class="kicker">MINHA CONTA</span><h2>Dados Cadastrais</h2><div class="account-details"><div><small>Nome</small><strong>${escapeHtml(state.user.name)}</strong></div><div><small>Login</small><strong>${escapeHtml(state.user.login||'')}</strong></div><div><small>E-mail</small><strong>${escapeHtml(state.user.email||'')}</strong></div><div><small>${customer?'Perfil':'Cargo'}</small><strong>${escapeHtml(customer?'Cliente':state.user.cargo||state.user.role)}</strong></div></div>${customer?`<form class="customer-address-form" id="customer-address-form"><h3>Endereço</h3><div class="checkout-grid"><label>CEP *<input name="zipCode" inputmode="numeric" value="${escapeHtml(address.zipCode||'')}" required></label><label>Rua *<input name="street" value="${escapeHtml(address.street||'')}" required></label><label>Número *<input name="number" value="${escapeHtml(address.number||'')}" required></label><label>Complemento<input name="complement" value="${escapeHtml(address.complement||'')}"></label><label>Bairro<input name="district" value="${escapeHtml(address.district||'')}"></label><label>Cidade *<input name="city" value="${escapeHtml(address.city||'')}" required></label><label>UF *<input name="state" maxlength="2" value="${escapeHtml(address.state||'')}" required></label></div><button class="primary full" type="submit">Salvar endereço</button><p class="form-error"></p></form>`:''}${profilePhotoContent()}${customer?'<button class="secondary full" id="orders">Ver minhas compras</button>':''}<button class="text-button full" id="logout">Sair da conta</button>`,'system-window-modal');bindProfilePhotoActions();$('#customer-address-form')?.addEventListener('submit',async event=>{event.preventDefault();const button=event.currentTarget.querySelector('[type=submit]');button.disabled=true;try{await api(`/api/stores/${slug}/customer/address`,{method:'PATCH',body:JSON.stringify(Object.fromEntries(new FormData(event.currentTarget)))});toast('Endereço salvo com sucesso.');button.disabled=false;}catch(error){event.currentTarget.querySelector('.form-error').textContent=error.message;button.disabled=false;}});$('#orders')?.addEventListener('click',showOrders);$('#logout').onclick=()=>{sessionStorage.removeItem(`token:${slug}`);sessionStorage.removeItem(`user:${slug}`);state.token='';state.user=null;closeModal();renderApp();toast('Você saiu da conta.');};}
async function removeProfilePhoto() { if(!confirm('A imagem anterior será removida. Deseja continuar?'))return;const photoOnly=Boolean($('.profile-photo-modal'));const button=$('#remove-profile-photo');button.disabled=true;button.textContent='Removendo...';try{await api(`/api/stores/${slug}/profile/photo`,{method:'DELETE'});state.user.photoUrl='';sessionStorage.setItem(`user:${slug}`,JSON.stringify(state.user));photoOnly?openProfilePhotoEditor():openAccount();toast('Foto removida. Agora você pode escolher uma nova imagem.');}catch(error){button.disabled=false;button.textContent='Remover ou alterar foto';toast(error.message,'error');} }
...
async function uploadProfileImage(image){try{const data=await api(`/api/stores/${slug}/profile/photo`,{method:'POST',body:JSON.stringify({image})});state.user.photoUrl=data.photoUrl;sessionStorage.setItem(`user:${slug}`,JSON.stringify(state.user));closeModal();renderApp();toast('Foto atualizada com sucesso.');}catch(error){toast(error.message,'error');}}
async function openPaymentData() {let card=null;try{card=(await api(`/api/stores/${slug}/customer/profile`)).card;}catch(error){toast(error.message,'error');}modal(`<span class="kicker">MINHA CONTA</span><h2>Dados de Pagamento</h2>${card?`<div class="saved-card"><small>Cartão cadastrado</small><strong>${escapeHtml(card.brand)} •••• ${escapeHtml(card.last4)}</strong><span>Validade ${escapeHtml(card.expiry)}</span></div>`:''}<form class="customer-card-form" id="customer-card-form"><h3>${card?'Substituir cartão':'Adicionar cartão de crédito'}</h3><p class="muted">Por segurança, não armazenamos o número completo nem o CVV.</p><div class="checkout-grid"><label>Nome no cartão *<input name="holderName" autocomplete="cc-name" required></label><label>Número do cartão *<input name="number" inputmode="numeric" autocomplete="cc-number" minlength="13" maxlength="23" required></label><label>Validade *<input name="expiry" autocomplete="cc-exp" placeholder="MM/AAAA" required></label></div><button class="primary full" type="submit">Salvar cartão com segurança</button><p class="form-error"></p></form>`,'system-window-modal');$('#customer-card-form').onsubmit=async event=>{event.preventDefault();const button=event.currentTarget.querySelector('[type=submit]');button.disabled=true;try{await api(`/api/stores/${slug}/customer/card`,{method:'PATCH',body:JSON.stringify(Object.fromEntries(new FormData(event.currentTarget)))});toast('Cartão cadastrado com segurança.');openPaymentData();}catch(error){event.currentTarget.querySelector('.form-error').textContent=error.message;button.disabled=false;}};}
...
async function openCorporate(page) {
  ...
  if (page === 'products') {
    ...
    let productImageData=''; const showProductImage=image=>{productImageData=image;$('#product-photo-preview').innerHTML=`<img src="${productImageData}" alt="Pré-visualização do produto">`;updateProductSubmit();};document.querySelectorAll('.product-photo-input').forEach(input=>input.onchange=async event=>{const file=event.currentTarget.files?.[0];if(!file)return;if(file.size>4*1024*1024){event.currentTarget.value='';return toast('A imagem deve ter no máximo 4 MB.','error');}showProductImage(await new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(reader.result);reader.onerror=reject;reader.readAsDataURL(file);}));});$('.product-camera-button').onclick=()=>openCamera({facingMode:'environment',onCapture:showProductImage});
...
function openCart() {
  const items=cartItems(); modal(`<span class="kicker">SEU CARRINHO</span><h2>${items.length ? `${items.length} ${items.length===1?'produto':'produtos'}` : 'Carrinho vazio'}</h2><div class="cart-list">${items.map(i=>`<div class="cart-item"><div class="cart-thumb" style="--art:${i.color}">${i.imageUrl?`<img src="${escapeHtml(i.imageUrl)}" alt="Imagem de ${escapeHtml(i.name)}">`:icon(i.icon,38)}</div><div><strong>${escapeHtml(i.name)}</strong><small>${money(i.price)}</small><div class="qty"><button data-minus="${i.id}">−</button><span>${i.quantity}</span><button data-plus="${i.id}">+</button></div></div><button class="remove" data-remove="${i.id}" aria-label="Remover">×</button></div>`).join('') || '<p class="muted">Adicione produtos para começar.</p>'}</div>${items.length?`<div class="cart-total"><span>Total</span><strong>${money(total())}</strong></div><button class="primary full" id="checkout">Continuar para pagamento</button>`:''}`, 'cart-modal system-window-modal');
  document.querySelectorAll('[data-minus]').forEach(b=>b.onclick=()=>{ if(--state.cart[b.dataset.minus]<=0)delete state.cart[b.dataset.minus];saveCart();openCart();}); document.querySelectorAll('[data-plus]').forEach(b=>b.onclick=()=>{const product=state.store.products.find(item=>item.id===b.dataset.plus);const stock=Math.max(0,Math.floor(Number(product?.stock)||0));if(state.cart[b.dataset.plus]>=stock)return toast('Quantidade máxima disponível em estoque.','error');state.cart[b.dataset.plus]++;saveCart();openCart();}); document.querySelectorAll('[data-remove]').forEach(b=>b.onclick=()=>{delete state.cart[b.dataset.remove];saveCart();openCart();}); $('#checkout')?.addEventListener('click',()=> state.user ? openCheckout() : openLogin(openCheckout));
}

...
function openAccount() { const customer=state.user.role==='cliente';modal(`<span class="kicker">MINHA CONTA</span><h2>Dados Cadastrais</h2><div class="account-details"><div><small>Nome</small><strong>${escapeHtml(state.user.name)}</strong></div><div><small>Login</small><strong>${escapeHtml(state.user.login||'')}</strong></div><div><small>E-mail</small><strong>${escapeHtml(state.user.email||'')}</strong></div><div><small>${customer?'Perfil':'Cargo'}</small><strong>${escapeHtml(customer?'Cliente':state.user.cargo||state.user.role)}</strong></div></div>${profilePhotoContent()}${customer?'<button class="secondary full" id="orders">Ver minhas compras</button>':''}<button class="text-button full" id="logout">Sair da conta</button>`,'system-window-modal');bindProfilePhotoActions();$('#orders')?.addEventListener('click',showOrders);$('#logout').onclick=()=>{sessionStorage.removeItem(`token:${slug}`);sessionStorage.removeItem(`user:${slug}`);state.token='';state.user=null;closeModal();renderApp();toast('Você saiu da conta.');};}
...
function openPaymentData() { modal(`<span class="kicker">MINHA CONTA</span><h2>Dados de Pagamento</h2><div class="notice-panel"><h3>Formas de pagamento</h3><p>Seus dados de pagamento serão apresentados e administrados com segurança nesta área.</p></div>`,'system-window-modal'); }

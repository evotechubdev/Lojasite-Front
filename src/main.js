...
function renderPortal() {
  ...
  const showcaseProducts=[
    {name:'Café Especial',category:'ALIMENTOS',price:38.9,icon:'speaker',color:'#efe4d4'},
    {name:'Fone Bluetooth',category:'TECNOLOGIA',price:149.9,icon:'headphones',color:'#dce8f5'},
    {name:'Bolsa Artesanal',category:'MODA',price:119.9,icon:'bag',color:'#f0dfdf'},
    {name:'Relógio Clássico',category:'ACESSÓRIOS',price:229.9,icon:'watch',color:'#e4e6df'},
    {name:'Câmera Compacta',category:'ELETRÔNICOS',price:899.9,icon:'camera',color:'#dfe5e8'},
    {name:'Carteira em Couro',category:'PRESENTES',price:79.9,icon:'wallet',color:'#eadfd5'}
  ];
  const showcaseCards=showcaseProducts.map(product=>`<article class="portal-product-card"><div class="portal-product-art" style="--portal-art:${product.color}"><span>${escapeHtml(product.category)}</span>${icon(product.icon,64)}</div><div><h3>${escapeHtml(product.name)}</h3><strong>${money(product.price)}</strong><button type="button">Ver produto</button></div></article>`).join('');
  const plans=systemConfig.plans.map((plan,index)=>`<article class="portal-plan${index===1?' featured':''}"><span>${index===1?'MAIS ESCOLHIDO':`PLANO ${escapeHtml(plan.name.toLocaleUpperCase('pt-BR'))}`}</span><h3>${escapeHtml(plan.name)}</h3><div class="plan-price"><small>R$</small><strong>${plan.monthlyPrice.toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2})}</strong><small>/mês</small></div><ul>${plan.benefits.map(item=>`<li>${escapeHtml(item)}</li>`).join('')}</ul><a href="#planos-contato" data-plan-contact="${escapeHtml(plan.name)}">Quero este plano</a></article>`).join('');
  $('#app').innerHTML = `<div class="portal"><header class="header portal-header"><a class="logo portal-logo portal-logo-only" href="${import.meta.env.BASE_URL}" aria-label="${escapeHtml(systemConfig.name)}"><img src="${import.meta.env.BASE_URL}imagens_pub/logo.png" alt="Logo ${escapeHtml(systemConfig.name)}"></a><nav><a href="#sua-loja">SUA LOJA</a><a href="#vitrine">VITRINE</a><a href="#tipos-venda">TIPOS DE VENDA</a><a href="#seguranca-lgpd">SEGURANÇA E LGPD</a><a href="#planos-contato">PLANOS E CONTATO</a></nav><a class="portal-access" href="#planos-contato">Acessar uma loja</a></header>
  <main>
  <section class="portal-hero portal-page-section" id="sua-loja"><div><span class="kicker">SUA LOJA, SEU ENDEREÇO, SUAS VENDAS</span><h1>Transforme seu estoque em uma loja online.</h1><p>Tenha um mostruário profissional, organize produtos e venda pelo próprio site — com uma experiência segura, prática e feita para o seu negócio.</p><a class="portal-primary" href="#vitrine">Conhecer a ${escapeHtml(systemConfig.name)} <span>↓</span></a></div><div class="portal-window"><div class="window-bar"><i></i><i></i><i></i><small>lojasite.com.br/sua-loja</small></div><div class="window-content"><span class="your-logo-placeholder">Sua Logo</span><div><small>SEU NEGÓCIO ONLINE</small><strong>Estoque, vitrine e vendas em <b>um só lugar</b></strong></div></div></div></section>
  <section class="portal-section portal-page-section showcase-section" id="vitrine"><div class="portal-title"><span class="kicker">VITRINE</span><h2>Produtos de diferentes setores, apresentados com padrão profissional.</h2><p>Uma experiência visual consistente para alimentos, moda, tecnologia, acessórios e muito mais.</p></div><div class="portal-showcase-wrap"><button id="showcase-previous" type="button" aria-label="Produtos anteriores">←</button><div class="portal-showcase" id="portal-showcase">${showcaseCards}</div><button id="showcase-next" type="button" aria-label="Próximos produtos">→</button></div></section>
  <section class="portal-section portal-page-section sales-section" id="tipos-venda"><div class="portal-title"><span class="kicker">TIPOS DE VENDA</span><h2>Venda do jeito que combina com sua operação.</h2><p>Do atendimento pessoal à compra totalmente online, sem obrigar o lojista a escolher apenas um caminho.</p></div><div class="sales-models"><article><b>01</b><span>SEM LOGIN</span><h3>Ordem de serviço pelo WhatsApp</h3><p>O visitante escolhe um produto, confirma a solicitação e é encaminhado diretamente ao WhatsApp operacional da loja. Simples, rápido e ideal para vendas consultivas.</p></article><article><b>02</b><span>COMPRA ONLINE</span><h3>Carrinho e pagamento no site</h3><p>O próprio cliente cria seu acesso, adiciona produtos ao carrinho e finaliza a compra com sua conta protegida, usando PIX ou cartão conforme a configuração da loja.</p></article></div></section>
  <section class="portal-section portal-page-section security-section" id="seguranca-lgpd"><div class="portal-title"><span class="kicker">SEGURANÇA E LGPD</span><h2>Proteção técnica com transparência, sem promessas ou selos inventados.</h2><p>A plataforma aplica controles concretos alinhados aos princípios de segurança, prevenção e necessidade da LGPD. Conformidade jurídica completa também depende das práticas de cada organização controladora.</p></div><div class="security-grid"><article><strong>Isolamento por organização</strong><p>Sessões validam a organização e clientes não podem consultar cadastros de outros clientes.</p></article><article><strong>Acesso mínimo necessário</strong><p>Funções corporativas são autorizadas por perfil e rotas sensíveis exigem autenticação.</p></article><article><strong>Proteções de aplicação</strong><p>Tokens internos criptografados, limites contra abuso, CSP, headers defensivos e validação de entradas.</p></article><article><strong>Privacidade por finalidade</strong><p>Dados públicos são separados de configurações privadas e credenciais permanecem somente no backend.</p></article></div><div class="security-references"><span>Referências oficiais:</span><a href="https://www.planalto.gov.br/ccivil_03/_ato2015-2018/2018/lei/l13709compilado.htm" target="_blank" rel="noopener">LGPD — Lei 13.709/2018</a><a href="https://www.gov.br/saude/pt-br/acesso-a-informacao/lgpd/principios/principios/" target="_blank" rel="noopener">Princípios da LGPD</a><a href="https://owasp.org/www-community/controls/Content_Security_Policy" target="_blank" rel="noopener">OWASP — CSP</a></div></section>
  <section class="portal-section portal-page-section plans-section" id="planos-contato"><div class="portal-title"><span class="kicker">PLANOS E CONTATO</span><h2>Comece a partir de R$ 49,90 por mês.</h2><p>Escolha uma estrutura compatível com o momento da sua loja.</p></div><div class="portal-plans">${plans}</div><div class="plans-bottom"><div class="plans-contact"><a class="email-button" data-new-store-email hidden>Falar por e-mail <span>↗</span></a><a class="whatsapp-button" data-whatsapp hidden>Falar pelo WhatsApp <span>↗</span></a></div><form id="portal-access-form"><span>lojasite.com.br/</span><input name="suffix" placeholder="sua_organizacao" pattern="[a-zA-Z0-9_-]+" required><button type="submit">Acessar minha loja</button></form></div></section>
  </main><footer class="portal-simple-footer"><img src="${import.meta.env.BASE_URL}imagens_pub/logo_dev.png" alt="Logo EVOTECHUB"><span>© EVOTECHUB 2026 - Todos os direitos reservados.</span></footer></div>`;
  ...
  portalLinks.forEach(link=>{const section=document.querySelector(link.getAttribute('href'));if(section)portalObserver.observe(section);});
  const showcase=$('#portal-showcase');
  const moveShowcase=direction=>{const card=showcase?.querySelector('.portal-product-card');if(!showcase||!card)return;const step=card.offsetWidth+18;const atEnd=showcase.scrollLeft+showcase.clientWidth>=showcase.scrollWidth-8;const atStart=showcase.scrollLeft<=8;showcase.scrollTo({left:direction>0?(atEnd?0:showcase.scrollLeft+step):(atStart?showcase.scrollWidth:showcase.scrollLeft-step),behavior:'smooth'});};

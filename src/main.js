...
function openLojaSitePopup(){
  ...
  overlay.innerHTML=`<section class="mini-modal lojasite-popup" role="dialog" aria-modal="true" aria-labelledby="lojasite-popup-title"><button class="mini-modal-close close" type="button" aria-label="Fechar">×</button><img src="${import.meta.env.BASE_URL}imagens/logo.png" alt="Logo LojaSite"><h3 id="lojasite-popup-title">LOJASITE</h3><p>Sua Loja Online!</p><button class="lojasite-portal-button" type="button">Portal LojaSite</button></section>`;
  $('#modal-root').append(overlay);
  const closePopup=()=>overlay.remove();
  overlay.querySelector('.mini-modal-close').onclick=closePopup;
  overlay.onclick=event=>{if(event.target===overlay)closePopup();};
  overlay.querySelector('.lojasite-portal-button').onclick=()=>{
    overlay.innerHTML=`<section class="mini-modal portal-confirm-popup" role="alertdialog" aria-modal="true" aria-labelledby="portal-confirm-title"><h3 id="portal-confirm-title">Deseja ser direcionado para o portal LOJASITE?</h3><div class="mini-modal-actions"><button class="secondary" data-portal-answer="no" type="button">Não</button><button class="lojasite-portal-button" data-portal-answer="yes" type="button">Sim</button></div></section>`;

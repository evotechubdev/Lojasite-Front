...
function renderApp() {
  ...
  const contactItems = `<div class="store-hours"><b>Horário de funcionamento</b><span>${escapeHtml(info.hours || 'Consulte a loja')}</span><div class="contact-icons">${info.whatsapp ? `<a class="contact-whatsapp" href="https://wa.me/${escapeHtml(info.whatsapp)}" target="_blank" rel="noopener" aria-label="Abrir WhatsApp" title="WhatsApp">${contactWhatsappIcon}</a>` : ''}${info.email ? `<a class="contact-email" href="mailto:${escapeHtml(info.email)}" aria-label="Enviar e-mail" title="E-mail">${contactMailIcon}</a>` : ''}</div></div>`;

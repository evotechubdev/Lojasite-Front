...
function renderApp() {
  ...
  const showStoreSection = section => {
    const target = ['novidades', 'produtos', 'nossa-loja'].includes(section) ? section : 'novidades';
    document.querySelectorAll('.store-page-section').forEach(element => { element.hidden = element.id !== target; });
    document.querySelectorAll('[data-store-section]').forEach(link => link.classList.toggle('active', link.dataset.storeSection === target));
    history.replaceState(null, '', `#${target}`);
    scrollTo({ top: 0, behavior: 'smooth' });
  };
  document.querySelectorAll('[data-store-section]').forEach(link => link.addEventListener('click', event => { event.preventDefault(); showStoreSection(link.dataset.storeSection); }));
  showStoreSection(location.hash.slice(1));

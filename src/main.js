...
function renderApp() {
  ...
  <main><section class="hero"><div class="hero-copy"><span class="kicker">NOVIDADES</span><h1>${escapeHtml(store.tagline)}</h1><p>Escolhas especiais, compra segura e entrega para todo o Brasil.</p><a href="#produtos">Ver produtos <span>↗</span></a></div>${heroVisual}</section>
  <section class="catalog" id="produtos"><div class="section-head"><div><span class="kicker">NOSSA SELEÇÃO</span><h2>Produtos em destaque</h2></div></div><div class="catalog-browser"><aside class="category-panel"><h3>Categorias</h3><div class="category-navigation" aria-label="Categorias">${categories.map(category=>`<button data-category="${escapeHtml(category)}" class="${state.category===category?'active':''}">${category==='Todos'?'Todas as Categorias':escapeHtml(category)}</button>`).join('')}</div></aside><div class="product-grid" id="products"></div></div></section>
